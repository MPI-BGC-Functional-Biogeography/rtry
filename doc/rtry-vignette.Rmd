...
R 4.0.2 was used to develop and build the `rtry` package, and this is the minimum version required to use the package. It is also recommended to use the latest version of RStudio when using the package.
...
To import the file `"input/8200.txt"` for preprocessing:

```R
TRYdata2 <- rtry_import("input/8200.txt", showOverview = FALSE)
```
...
```R
TraitsAuxNames <- rtry_import("output/Traits_auxilliary_Data_incl_Names.csv", separator = ",", encoding = "UTF-8", quote = "\"")
```

```R
## input: output/Traits_auxilliary_Data_incl_Names.csv
## dim:   1374 6 
