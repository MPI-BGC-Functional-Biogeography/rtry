...
An R package is developed to facilitate the preprocessing of the TRY data.
...
![image-20201214084248082](C:\Users\hlam\AppData\Roaming\Typora\typora-user-images\image-20201214084248082.png)
...
```R
rtry_select_row(input = "", ..., getAuxiliary = FALSE, rmDuplicates = FALSE, showOverview = TRUE)
```
...
```R
TRYdata_selectRow <- rtry_select_row(TRYdata_select_col, (TraitID > 0) | (DataID %in% c(59, 60, 61, 6601, 327, 413, 1961, 210, 308)))
```
...
![image-20210212003032439](C:\Users\hlam\AppData\Roaming\Typora\typora-user-images\image-20210212003032439.png)



Once the filtering criteria are identified, perform the filtering using ```rtry_filter()```.

(a) Manually enters the values for exact match (by default also case sensitive) filtering:

```R
tmp2 <- rtry_filter(tmp, OrigValueStr, c("2.5", "juvenile", "Juvenile", "juvenile, 6 weeks"))
```
...
```R
tmp3 <- rtry_filter(tmp, OrigValueStr, c("2.5", "juvenile"), caseSensitive = FALSE, exactMatch = FALSE)
```
...
![image-20210212002735840](C:\Users\hlam\AppData\Roaming\Typora\typora-user-images\image-20210212002735840.png)
