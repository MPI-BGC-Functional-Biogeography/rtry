...
tmp_unfiltered <- rtry_explore(tmp_unfiltered, OriglName, OrigValueStr, OrigUnitStr, StdValue, Comment, sortBy = OrigValueStr)
```
...
![workflow_general_filter_mature_explore_before](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_general_filter_mature_explore_before.png){width=80%}
...
tmp_filtered <- rtry_explore(tmp_filtered, OriglName, OrigValueStr, OrigUnitStr, StdValue, Comment, sortBy = OrigValueStr)
```
...
![workflow_general_filter_mature_explore_after](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_general_filter_mature_explore_after.png){width=80%}
...
```R
# Criteria
# 1. DataID equals to 7222, 7223 and 6598
...
![workflow_general_explore_aux_filtered_extremes](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_general_explore_aux_filtered_extremes.png)



### Filter negative values

When dealing with data related to the leaf specific area (SLA), it is important to make sure there is no negative values in the dataset, since it is not possible to have negative area. Therefore, it is also possible to use the filtering function provided in the `rtry` package to remove the negative values.
...
tmp_unfiltered <- rtry_explore(tmp_unfiltered, DataName, OriglName, OrigValueStr, OrigUnitStr, StdValue, Comment, sortBy = StdValue)
```
...
workdata <- rtry_filter(workdata, (DataID %in% c(6582, 6583, 6584)) & (StdValue < 0))
```
...
tmp_filtered <- rtry_explore(tmp_filtered, DataName, OriglName, OrigValueStr, OrigUnitStr, StdValue, Comment, sortBy = StdValue)
```
...
Note: Since there are no negative SLA values in this case, no data has been removed.
...
tmp_unfiltered <- rtry_explore(tmp_unfiltered, OriglName, OrigValueStr, OrigUnitStr, StdValue, Comment, sortBy = StdValue)
```
...
tmp_filtered <- rtry_explore(tmp_filtered, OriglName, OrigValueStr, OrigUnitStr, StdValue, Comment, sortBy = StdValue)
```
...
For demonstration purpose, the following command filters the longitude smaller than 10.00000 or larger than 50.00000.

```R
# Criteria
# 1. DataID equals to 60
# 2. StdValue smaller than 10 or larger than 50
workdata <- rtry_filter(workdata, (DataID %in% 60) & (StdValue < 10 | StdValue > 50))
```
...
In general, the transformation will be performed on the `TraitID`, `TraitName` and `UnitName`, meaning to get the name of these two columns as part of the name of the output column (`names_from`), and then use the `StdValue` and `ErrorRisk` as the cell values (`values_from`). Also, the mean function is to be applied to each cell in the output (`values_fn`). To do so, use the following command:

```R
# Perform wide table transformation on TraitID, TraitName and UnitName
# With cell values to be the mean values calculated for StdValue and ErrorRisk
workdata_wider_traits <- rtry_trans_wider(workdata, names_from = c(TraitID, TraitName, UnitName), values_from = c(StdValue, ErrorRisk), values_fn = list(StdValue = mean, ErrorRisk = mean))
```
...
Immediately, it can be seen that the number of columns is increased from 17 to 22, with new columns named for example "StdValue_3115_Leaf area per leaf dry mass (specific leaf area, SLA or 1/LMA): petiole excluded_mm2 mg-1".
...
Despite the above function works as expected, it is important to note that transforming the data using `TraitID` or `TraitName` can sometimes cause duplicate entries on `ObservationID`. In such cases, the mean is calculate for `StdValue` and `ErrorRisk`. In order to avoid this, run the `rtry_trans_wider()` on the `DataID` and `DataName` instead:

```R
# Perform wide table transformation on DataID and DataName
# With cell values to be the mean values calculated for StdValue and ErrorRisk
workdata_wider_data <- rtry_trans_wider(workdata, names_from = c(DataID, DataName), values_from = c(StdValue, ErrorRisk), values_fn = list(StdValue = mean, ErrorRisk = mean))
```
...
This time, the column numbers is increased from 17 to 27, with new columns named for example "StdValue_6582_SLA: petiole excluded".

![workflow_general_trans_wider_data](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_general_trans_wider_data.png){width=100%}
...
rtry_export(workdata_wider_traits, "output/workdata_wider_traits.csv")
rtry_export(workdata_wider_data, "output/workdata_wider_data.csv")
```

```R
## New directory created at: output 
## File saved at: output/workdata_wider_traits.csv
## File saved at: output/workdata_wider_data.csv
```
