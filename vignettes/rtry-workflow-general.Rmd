...
As of July 2019, the TRY database comprised 588 data sets from 765 data contributors (Kattge et al. [2019](https://onlinelibrary.wiley.com/doi/full/10.1111/gcb.14904) GCB). To keep track of potential duplicate entries, a unique identifier `OrigObsDataID` is assigned when the probability is high that certain trait records have previously been contributed (Kattge el al. 2020). With the help of this `OrigObsDataID`, the `rtry` package provides a function `rtry_remove_dup` for users to easily remove the duplicates from the data for further processing.
