...
This vignette is intended to demonstrate how to use the `rtry` package to preprocess data exported from the TRY database, from importing and exploring the data to binding multiple datasets, to selecting, filtering specific data using user-defined criteria and removing duplicates, and finally exporting the preprocessed data.
...
With this exploration and the way it is sorted, it is clear that in the `TRYdata1`: (1) there are two traits, with the `TraitID` equals to 3115 and 3116; (2) what types of auxiliary data could be found with the list of `DataID` and `DataName`; and (3) within each trait, whether or not similar data exists (`DataID` and `DataName`).
...
If used the `View(TRYdata2_explore_aux)` function, it can be seen that in the `TRYdata2`:  (1) there is only one trait, with the `TraitID` equals to 3117; (2) what types of auxiliary data could be found with the list of `DataID` and `DataName`; and (3) within this trait, whether or not similar data exists.
...
The `rtry_bind_row()` function takes two arguments `...` and `showOverview`. It takes a sequence of data and combines them by rows. Note: The data have to have the same number of columns and the same sequence of column names.
...
The `rtry_select_row()` takes five arguments `input`, `...`, `getAuxiliary`, `rmDuplicates` and `showOverview` to select specified rows based on the specified criteria from the imported data. Note that the argument `getAuxiliary` makes use of the `ObservationID` provided within the TRY data to select the whole observation including all corresponding auxiliary data during the row extraction process.

For our example we will select all trait records, but only relevant auxiliary data.

First identify relevant auxiliary data using the `rtry_explore` function.
...
We retrieve six `DataID`s with trait records and 325 `DataID`s with auxiliary information. For further data preprocessing we decide to select the following `DataID`s to contain relevant information:
...
To select all trait records and only the auxiliary data of interest:
...
It is recommended to backup the data at some stages of the preprocessing, best before filtering according to different attributes.
...
First, check the `DataID` that contains information about plant development status, i.e. `413`, then check the different states of the `OrigValueStr` using the functions `rtry_select_row()` and `rtry_explore()`. The temporary data is saved as `tmp_unfiltered`.
...
By sorting the exploration by `OrigValueStr`, it is clear that what types of developmental state exist in the dataset. This way it is possible to set the criteria and start the filtering process using `rtry_filter()`.

![workflow_general_filter_juvenile_explore_before](workflow_general_filter_juvenile_explore_before.png){width=100%}

Filter juvenile plants and saplings using `rtry_filter`.

```R
# Criteria
# 1. DataID equals to 413
# 2. OrigValueStr equals to "juvenile" or "saplings"
workdata <- rtry_filter(workdata, (DataID %in% 413) & (OrigValueStr %in% c("juvenile", "saplings")), baseOn = ObservationID)
```
...
Once the filtering is completed, double check the `workdata` the functions `rtry_select_row` and `rtry_explore`. The temporary data are saved as `tmp_filtered`.
...
It is clear that all the juvenile and sapling plants were filtered away as expected.

![workflow_general_filter_juvenile_explore_after](workflow_general_filter_juvenile_explore_after.png){width=100%}

To further confirm if the trait and/or other auxiliary data were also removed accordingly, use again the `rtry_explore` function.
...
Compare with the `workdata_explore_aux`, it can be seen that the number of traits and auxiliary data also decreased as expected. We can see as well, that observations without a record for `DataID` 413 have not been removed by the filter function, assuming no information would rather follow the recommended measurement protocol - here measuring traits on mature plants.

![workflow_general_explore_aux_filtered_juvenile](workflow_general_explore_aux_filtered_juvenile.png){width=100%}
...
For demonstration purpose, the following command filters the latitude smaller than 40 or when such information is missing, i.e. `NA`.

```R
# Criteria
# 1. DataID equals to 59
# 2. StdValue smaller than 40 or NA
workdata <- rtry_filter(workdata, (DataID %in% 59) & (StdValue < 40 | is.na(StdValue)), baseOn = ObservationID)
```
...
For demonstration purpose, the following command filters the longitude smaller than 10 or larger than 60 or when such information is missing, i.e. `NA`.

```R
# Criteria
# 1. DataID equals to 60
# 2. StdValue smaller than 10 or larger than 60 or NA
workdata <- rtry_filter(workdata, (DataID %in% 60) & (StdValue < 10 | StdValue > 60 | is.na(StdValue)), baseOn = ObservationID)
```
...
From the exploration result, it is clear that only the latitude values in between 10 and 60 remains.
...
In the examples below the argument `baseOn` is set to `ObsDataID`. These examples are supposed to filter non-representative trait records, while keeping the rest of the observation, because the observation might contain records for other traits with rather representative measurements.
...
![workflow_general_explore_aux_filtered_observations](workflow_general_explore_aux_filtered_observations.png){width=100%}

Now that the filtering criteria has been identified, the user might decided to only remove the trait records for `DataID` 7222, 7223 and 6598, while keeping the rest of the observation, because it might contain representative records for other traits. In this case, use the `rtry_filter` function with `baseOn` specified to `ObsDataID`:

```R
# Criteria
# 1. DataID equals to 7222, 7223 or 6598
workdata <- rtry_filter(workdata, DataID %in% c(7222, 7223, 6598), baseOn = ObsDataID)
```
...
workdata <- rtry_filter(workdata, (DataID %in% c(6582, 6583, 6584)) & (StdValue < 1), baseOn = ObsDataID)
```
...
workdata <- rtry_filter(workdata, ErrorRisk >= 4, baseOn = ObsDataID)
```
...
As of July 2019, the TRY database comprised 588 data sets from 765 data contributors (Kattge et al. [2019](https://onlinelibrary.wiley.com/doi/full/10.1111/gcb.14904) GCB). To keep track of potential duplicate entries, an unique identifier known as `OrigObsDataID` is assigned when certain observation data has previously been contributed. With the help of this `OrigObsDataID`, the `rtry` package provides a function `rtry_remove_dup()` for users to easily remove the duplicates from the data for further processing.
