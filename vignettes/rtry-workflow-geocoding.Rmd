---
title: "Using rtry for (reverse) geocoding"
output:
  rmarkdown::html_vignette:
    ...
    # toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Using rtry for (reverse) geocoding}
...
This vignette is intended to demonstrate how to use the `rtry_geocoding()` and `rtry_revgeocoding()` within the `rtry` package to perform geocoding and reverse geocoding for a list of locations or coordinates.
...
The functions `rtry_geocoding()` and `rtry_revgeocoding` are based on Nominatim, a search engine for OpenStreetMap (OSM) data. For details, please refer to: https://wiki.openstreetmap.org/wiki/Nominatim.

Note that the geo-reference system used is WGS84 projection.



## Prepare environment

Make sure you have the `rtry` package installed. If not, you may refer to the "Installation Guide" inside the `docs` directory. Note that an example workflow is provided in the `examples` directory.

Load the required packages using the commands:

```R
library(rtry)	# load the rtry package
library(dplyr)	# load the dplyr pacakge, needed for piping (%>%)
```
...
```R
input_locations <- rtry_import("input/TRY_Sample_Locations.csv", separator = ",", quote = "\"")
```

```R
## input: input/TRY_Sample_Locations.csv
## dim:   29 3
## ls:    Country Country code Location
```

![workflow_geocoding_input](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_geocoding_locations.png)
...
```R
## [1] "Hajdudorog, Hungary" "Diosd, Hungary"      "Fot, Hungary"       
## [4] "Bőcs, Hungary" "Regec, Hungary"      "Eger, Hungary"
```
...
![workflow_geocoding_output](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_geocoding_coordinates.png)
...
# Export into .csv
rtry_export(input_locations, "output/TRY_Sample_Locations_Coordinates.csv")
```

![workflow_geocoding_input_coordinates](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_geocoding_locations_coordinates.png)
...
```R
input_coordinates <- rtry_import("input/TRY_Sample_Coordinates.csv", separator = ",", quote = "\"")
```

```R
## input: C:/Users/hlam/ownCloud/TRY_R/rtry_share_obg/examples/input/TRY_Sample_Coordinates.csv
## dim:   29 2 
## ls:    Latitude Longitude 
```

![workflow_revgeocoding_coordinates](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_revgeocoding_coordinates.png)
...
In order to apply the function `rtry_revgeocoding()` to the `input_lat_lon`, use `lapply()`, and please remember to change the email address into your own email address.
...
![workflow_revgeocoding_locations](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_revgeocoding_locations.png){width=100%}
...
# Export into .csv
rtry_export(input_coordinates, "output/TRY_Sample_Coordinates_Locations.csv")
```

![workflow_revgeocoding_coordinates_locations](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_revgeocoding_coordinates_locations.png)
