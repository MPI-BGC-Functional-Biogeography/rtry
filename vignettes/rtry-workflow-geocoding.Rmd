...
The functions `rtry_geocoding()` and `rtry_revgeocoding` are based on Nominatim, a search engine for OpenStreetMap (OSM) data.  the data provided are free to use for any purpose, including commercial use, note that they are governed by the ODbL (https://wiki.osmfoundation.org/wiki/Licence). For details, please refer to: https://wiki.openstreetmap.org/wiki/Nominatim.
...
![workflow_geocoding_input](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_geocoding_locations.png){width=40%}
...
output_coordinates <- lapply(input_addresses, function(address) {
  ...
  geocode_output <- rtry_geocoding(address, email = "jens.kattge@bgc-jena.mpg.de")
...
![workflow_geocoding_output](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_geocoding_coordinates.png){width=40%}
...
![workflow_geocoding_input_coordinates](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_geocoding_locations_coordinates.png){width=60%}
...
![workflow_revgeocoding_coordinates](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_revgeocoding_coordinates.png){width=30%}
...
output_locations <- apply(input_lat_lon, 1, function(lat_lon) {
  # Calling the Nominatim OpenStreetMap API
  rev_geocode_output <- rtry_revgeocoding(lat_lon, email = "jens.kattge@bgc-jena.mpg.de")
...
![workflow_revgeocoding_coordinates_locations](C:\Users\hlam\ownCloud\TRY_R\rtry\doc\image\workflow_revgeocoding_coordinates_locations.png){width=60%}
