...
When a memory issue occurs, users could either use a machine with more memory (RAM) installed, or they could request smaller datasets (instead of one large dataset) and import the datasets into RStudio separately.
...
Else,  if user downloaded the latest source package (`.tar.gz`) of `rtry` from the GitHub repository or Nextcloud:
...
The `rtry` package is a compilation of functions developed to support the preprocessing of trait data, foremost if received via the TRY database. In `rtry`, a function is a combination of R commands used with specified attributes to enable easy application. Sometimes these attributes are specified even for the structure and column names used in the data released from TRY.
...
```R
# For the documentation of a specific dataset (e.g. `TRYdata_15160` or `locations`)
...
## [1] "/Library/Frameworks/R.framework/Versions/4.0/Resources/library/rtry/testdata/TRYdata_15160.txt"
## [1] "/Library/Frameworks/R.framework/Versions/4.0/Resources/library/rtry/testdata/locations.csv"

# Expected return on a Windows OS is similar to this:
## [1] "C:/Program Files/R/R-4.0.3/library/rtry/testdata/TRYdata_15160.txt"
## [1] "C:/Program Files/R/R-4.0.3/library/rtry/testdata/locations.csv"
```
