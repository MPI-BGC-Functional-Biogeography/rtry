...
Inside the `rtry` package, we use a function naming convention where each function begins with the prefix `rtry_` followed by what the specific function does.
...
This function, by default, imports the data file (.txt) exported from the TRY database as a data.table for further processing. It can also be used to import other file formats, such as .csv files with comma as separator.
...
| `quote`        | Default `""` reads the fields as is. If the fields in the data file are by a double quote, use `"\""` instead |
| `showOverview` | Default `TRUE` displays the input path, the dimension and the column names of the imported data |



**Example 1**

To import the file `input/TRYdata_15160.txt` and `input/TRYdata_15161.txt` for preprocessing:

```R
data1 <- rtry_import("input/TRYdata_15160.txt")
```
...
```R
data2 <- rtry_import("input/TRYdata_15161.txt")
```
...
To import a CSV file (comma separated values), specify the separator to ",":
...
This function takes the data table imported by `rtry_import()` and converts it into a grouped data table based on the specified column names. Note that the output data are grouped by the first attribute if not specified using the parameter `sortBy`. To provide a first understanding of the data, an additional column is added to show the total count within each group.
...
To explore the unique values in `data1` based on the attributes `TraitID`, `TraitName`, and grouped by the first attribute, i.e. `TraitID`:

```R
data1_explore <- rtry_explore(data1, TraitID, TraitName)
```
...
To explore the unique values in `data1` based on the attributes `AccSpeciesID`, `DataID`, `DataName`, `TraitID` and `TraitName`, and sorted by `DataName`:

```R
data1_explore_accSpecies <- rtry_explore(data1, AccSpeciesID, DataID, DataName, TraitID, TraitName, sortBy = DataName, showOverview = FALSE)
```
...
This function takes a sequence of data imported by `rtry_import()` and combines them by columns. The data should share the same number of rows.
...
This function takes a sequence of data imported by `rtry_import()` and combines them by rows. The data should share the same number of columns and column names.
...
To combine the data `data1` and `data2` together by rows:

```R
data <- rtry_bind_row(T, data2)
```
...
This function merges two data frames based on the row names of the specified common column (by default: `ObservationID`) and returns all rows from both tables, join records from the left (`x`) which have matching keys in the right data frame (`y`) for further processing. In order words, this function performs an outer join on the two provided data frames, i.e. the join table will contain all records from both the tables.
...
This function selects specified columns from the imported data and saves them in a new data table. An alternative to achieve this purpose would be `rtry_rm_col()` where the columns to be removed are specified instead.
...
| `showOverview` | Default `TRUE` displays the dimension and column namesof the selected columns |



**Example 1**

To select (or say extract) the columns `ObsDataID`, `ObservationID`, `AccSpeciesID`, `AccSpeciesName`, `ValueKindName`, `TraitID`, `TraitName`, `DataID`, `DataName`, `OriglName`, `OrigValueStr`, `OrigUnitStr`, `StdValue`, `UnitName`, `OrigObsDataID`, `ErrorRisk`, `Comment` within the `data`:

```R
data_selectColumns <- rtry_select_col(data, ObsDataID, ObservationID, AccSpeciesID, AccSpeciesName, ValueKindName, TraitID, TraitName, DataID, DataName, OriglName, OrigValueStr, OrigUnitStr, StdValue, UnitName, OrigObsDataID, ErrorRisk, Comment)
```
...
To select several neighboring columns, e.g. `TraitID` to `ValueKindName`:

```R
data_selectNeighborColumns <- rtry_select_col(data, TraitID:ValueKindName)
```

```R
## dim:   6409 8 
## col:   TraitID TraitName DataID DataName OriglName OrigValueStr OrigUnitStr ValueKindName 
```
...
This function selects rows based on specified criteria and `ObservationID` from the imported data for further processing.
...
To select (or say filter) the rows where `TraitID` is larger than 0 (`TraitID > 0`) or where `DataID` is 59, 60, 61, 6601, 327, 413 or 1961 (`DataID %in% c(59, 60, 61, 6601, 327, 413, 1961)`) within the data `TRYdata_select_col`:

```R
data_selectRow <- rtry_select_row(data_selectColumns, (TraitID > 0) | (DataID %in% c(59, 60, 61, 6601, 327, 413, 1961)))
```
...
```R
traitID6_auxiliary <- rtry_select_row(data, TraitID %in% c(3115), getAuxiliary = TRUE, rmDuplicates = TRUE)
```
...
```R
outlier_removal <- rtry_select_row(data, ErrorRisk < 4 | DataID %in% c(59, 60, 61, 6601, 327, 413, 1961))
```
