context("Merge two data frames - left join (rtry_merge_col)")
...
test_that("rtry_merge_col handles missing specified common column", {
  ...
  expect_message(rtry_merge_col(df1, df2), message)
})


test_that("rtry_merge_col handles empty or missing argument", {
  message = "Please specify the two data frames you would like to merge."

  expect_message(rtry_merge_col(), message)
  expect_message(rtry_merge_col(x = x), message)
})
